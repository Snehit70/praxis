...
const STATE_FILE = path.join(path.resolve(import.meta.dir, '..'), 'scraper-state.json');
...
export class AdaptiveRateLimiter {
    ...
    isRateLimitError(error: unknown): boolean {
        ...
        if (candidate.code === 'ECONNRESET' || candidate.code === 'ETIMEDOUT' || candidate.code === 'ECONNREFUSED') {
            return true;
        }
        return false;
    }
